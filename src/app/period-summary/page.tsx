...
export default function PeriodSummaryPage() {
    ...
    // UK Property state (dates kept here for submission logic)
    ...
    // Helper for Self-Employment dates (using PeriodDates interface)
    const handleSEDateChange = (field: keyof PeriodDates, value: string) => {
        setFormData((prev) => ({
            ...prev,
            periodDates: {
                ...prev.periodDates,
    ...
    // Helper for Property dates (using propertyDates state)
    const handlePropertyDateChange = (field: "fromDate" | "toDate", value: string) => {
        setPropertyDates((p) => ({ ...p, [field]: value }));
    };
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        // Basic validation based on summary type
        ...
        } else { // UK Property or Foreign Property
    ...
    // NEW: small renderer for single numeric property fields with space optimization
    const renderNumberField = (
        label: string,
        value: string,
        onChange: (v: string) => void,
        hint?: string,
        key?: string // Added key for easier grouping/mapping if needed, though not strictly required inside the grouping div
    ) => (
    ...
    return (
        <StepLayout title="Period Summary" backHref="/obligations" next={null}>
            ...
            <form onSubmit={onSubmit} className="space-y-6">
                {/* Unified Dates / Tax Year Panel */}
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-lg font-semibold mb-4">
                        Period Dates and Tax Year
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                Start Date {summaryType === "selfEmployment" && <span className="text-red-500">*</span>}
                            </label>
                            <input
                                type="date"
                                className="w-full p-2 border rounded-md"
                                value={
                                    summaryType === "selfEmployment"
                                        ? formData.periodDates.periodStartDate || ""
                                        : propertyDates.fromDate
                                }
                                onChange={(e) =>
                                    summaryType === "selfEmployment"
                                        ? handleSEDateChange("periodStartDate", e.target.value)
                                        : handlePropertyDateChange("fromDate", e.target.value)
                                }
                                required={summaryType === "selfEmployment"}
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                End Date {summaryType === "selfEmployment" && <span className="text-red-500">*</span>}
                            </label>
                            <input
                                type="date"
                                className="w-full p-2 border rounded-md"
                                value={
                                    summaryType === "selfEmployment"
                                        ? formData.periodDates.periodEndDate || ""
                                        : propertyDates.toDate
                                }
                                onChange={(e) =>
                                    summaryType === "selfEmployment"
                                        ? handleSEDateChange("periodEndDate", e.target.value)
                                        : handlePropertyDateChange("toDate", e.target.value)
                                }
                                required={summaryType === "selfEmployment"}
                            />
                        </div>
                    </div>
                    {/* Tax Year field is present for all tabs, but only required for Property */}
                    <div className="mt-4">
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                            Tax Year (e.g. 2024-25) {summaryType !== "selfEmployment" && <span className="text-red-500">*</span>}
                        </label>
                        <input
                            type="text"
                            className="w-full p-2 border rounded-md"
                            ...
                            {expandedPanels.disallowableExpenses && (
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {renderInputField(
                                        ...
                                        "Cost of Goods (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "paymentsToSubcontractorsDisallowable",
                                        "Payments to Subcontractors (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "wagesAndStaffCostsDisallowable",
                                        "Wages and Staff Costs (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "carVanTravelExpensesDisallowable",
                                        "Car/Van Travel Expenses (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "premisesRunningCostsDisallowable",
                                        "Premises Running Costs (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "maintenanceCostsDisallowable",
                                        "Maintenance Costs (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "adminCostsDisallowable",
                                        "Admin Costs (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "businessEntertainmentCostsDisallowable",
                                        "Business Entertainment (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                    ...
                                    {renderInputField(
                                        ...
                                        "Interest on Bank/Loans (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "financeChargesDisallowable",
                                        "Finance Charges (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                                        "irrecoverableDebtsDisallowable",
                                        "Irrecoverable Debts (Disallowable)"
                                    )}
                                    {renderInputField(
                                        "periodDisallowableExpenses",
                ...
                {/* UK Property UI - UPDATED FOR 2 FIELDS PER ROW */}
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {renderNumberField(
                                    ...
                                    ukIncome.premiumsOfLeaseGrant,
                                    (v) => setUkIncome((s) => ({ ...s, premiumsOfLeaseGrant: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Reverse Premiums",
                                    ukIncome.reversePremiums,
                                    (v) => setUkIncome((s) => ({ ...s, reversePremiums: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Period Amount (Total rents)",
                                    ukIncome.periodAmount,
                                    (v) => setUkIncome((s) => ({ ...s, periodAmount: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Tax Deducted",
                                    ukIncome.taxDeducted,
                                    (v) => setUkIncome((s) => ({ ...s, taxDeducted: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Other Income",
                                    ukIncome.otherIncome,
                                    (v) => setUkIncome((s) => ({ ...s, otherIncome: v })),
                                    "£"
                                )}
                                {/* Padder for odd number of fields if needed, but keeping it flexible */}
                            </div>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {renderNumberField(
                                    "Premises Running Costs",
                                    ukExpenses.premisesRunningCosts,
                                    (v) => setUkExpenses((s) => ({ ...s, premisesRunningCosts: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Repairs & Maintenance",
                                    ukExpenses.repairsAndMaintenance,
                                    (v) => setUkExpenses((s) => ({ ...s, repairsAndMaintenance: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Financial Costs",
                                    ukExpenses.financialCosts,
                                    (v) => setUkExpenses((s) => ({ ...s, financialCosts: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Professional Fees",
                                    ukExpenses.professionalFees,
                                    (v) => setUkExpenses((s) => ({ ...s, professionalFees: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Cost Of Services",
                                    ukExpenses.costOfServices,
                                    (v) => setUkExpenses((s) => ({ ...s, costOfServices: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Other Expenses",
                                    ukExpenses.other,
                                    (v) => setUkExpenses((s) => ({ ...s, other: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Residential Financial Cost",
                                    ukExpenses.residentialFinancialCost,
                                    (v) => setUkExpenses((s) => ({ ...s, residentialFinancialCost: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Travel Costs",
                                    ukExpenses.travelCosts,
                                    (v) => setUkExpenses((s) => ({ ...s, travelCosts: v })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Residential Financial Costs Carried Forward",
                                    ukExpenses.residentialFinancialCostsCarriedForward,
                                    (v) =>
                                        setUkExpenses((s) => ({
                                            ...s,
                                            residentialFinancialCostsCarriedForward: v,
                                        })),
                                    "£"
                                )}
                                {renderNumberField(
                                    "Consolidated Expenses",
                                    ukExpenses.consolidatedExpenses,
                                    (v) => setUkExpenses((s) => ({ ...s, consolidatedExpenses: v })),
                                    "£"
                                )}
                            </div>
                        </div>
                ...
                {/* Foreign Property UI - UPDATED FOR 2 FIELDS PER ROW */}
                {summaryType === "foreignProperty" && (
                    <>
                        {foreignProperties.map((entry, idx) => (
                            <div key={idx} className="bg-white p-6 rounded-lg shadow-md">
                                <h2 className="text-lg font-semibold mb-4">
                                    Foreign Property #{idx + 1}
                                </h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {/* Country Code and Checkbox in first row */}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">
                                    ...
                                    <div className="flex items-center space-x-2 pt-6"> {/* Added pt-6 for vertical alignment */}
                                        <input
                                            id={`ftcr-${idx}`}
                                            type="checkbox"
                                        ...
                                        />
                                        <label htmlFor={`ftcr-${idx}`} className="text-sm">
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {renderNumberField(
                                            "Premiums Of Lease Grant",
                                            entry.income.premiumsOfLeaseGrant || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            ...
                                            entry.income.otherPropertyIncome || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            ...
                                            entry.income.foreignTaxPaidOrDeducted || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            ...
                                            entry.income.specialWithholdingTaxOrUkTaxPaid || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {renderNumberField(
                                            "Premises Running Costs",
                                            entry.expenses.premisesRunningCosts || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            "Repairs & Maintenance",
                                            entry.expenses.repairsAndMaintenance || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            "Professional Fees",
                                            entry.expenses.professionalFees || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            "Residential Financial Cost",
                                            entry.expenses.residentialFinancialCost || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            ...
                                            entry.expenses.broughtFwdResidentialFinancialCost || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
                                        ...
                                        {renderNumberField(
                                            "Consolidated Expenses",
                                            entry.expenses.consolidatedExpenses || "",
                                            (v) => {
                                                const copy = [...foreignProperties];
